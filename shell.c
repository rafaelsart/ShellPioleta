...
*/
int Terminal_rodaLinhaComando (char *Diretorio, char **Parametro, int numParametros, int runForeground) {
	//Variáveis
	...
	//pwd
	if(strcmp(Parametro[0], "pwd") == 0) {
		//Recupera e imprime diretório atual
		getcwd(Diretorio, 1000);
		printf("%s\n", Diretorio);
	}
	//exit
...
int Terminal_InterpretaLinhaComando (char *LinhaComando, char **LinhasComando, char *Diretorio) {
	...
	//Roda comando
	Terminal_rodaLinhaComando(Diretorio, Parametro, numParametros, runForeground);
